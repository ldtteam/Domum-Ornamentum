...
public class FramedLightItemModelProvider implements DataProvider
{
    private final DataGenerator generator;

    public FramedLightItemModelProvider(final DataGenerator generator)
    {
    ...
    public void run(@NotNull CachedOutput cache) throws IOException
    {
        ...
        for (FramedLightBlock framedLight : ModBlocks.getInstance().getFramedLights())
        {
            final String modelLocation = new ResourceLocation(Constants.MOD_ID, "block/framed_light/" + Objects.requireNonNull(framedLight.getRegistryName()).getPath()).toString();

            modelJson.setParent(modelLocation);

            if (framedLight.getRegistryName() == null)
                continue;

            final String name = framedLight.getRegistryName().getPath();
    ...
    {
        return "Framed Light Item Model Provider";
    }
}
