...
public class FancyDoorsBlockStateProvider implements DataProvider
{
    ...
    {
        createBlockstateFile(cache, ModBlocks.getInstance().getFancyDoor());
    }

    private void createBlockstateFile(final CachedOutput cache, final FancyDoorBlock shingle) throws IOException
    {
        ...
        {
            for (FancyDoorType typeValue : FancyDoorBlock.TYPE.getPossibleValues())
            {
                for (DoubleBlockHalf halfValue : HALF.getPossibleValues())
                {
                    for (boolean openValue : OPEN.getPossibleValues())
                    {
                        for (final DoorHingeSide hingeValue : HINGE.getPossibleValues())
                        {
                            ...
                            final String modelLocation = Constants.MOD_ID + ":block/doors/fancy/door_" + (halfValue == DoubleBlockHalf.UPPER ? "top_" : "") + typeValue.getSerializedName();
